...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderResult = () => {
    const data = new FormData(form); const typeValue = data.get('type'); const placeValue = data.get('place'); const type = typeValue ? form.elements.type.selectedOptions[0].textContent : '상품권 형태 미선택'; const place = placeValue ? form.elements.place.selectedOptions[0].textContent : '사용 장소 미선택'; const region = String(data.get('region') || '').trim();
    ...
    let guide = '/where/'; if (placeValue === 'online') guide = '/online/'; else if (typeValue === 'paper') guide = '/paper/'; else if (typeValue === 'digital') guide = '/digital/';
